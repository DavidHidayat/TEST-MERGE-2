...
{       
    ...
    if($sUserId != '')
    {
        ...
        if($_SESSION['EPSAuthority'] != 'EPSUploadPrScreen')
        {       
        ?>
...
<html>
    <head>
        ...
        var mainLayout = function(){
            var toolbarTop = new Ext.Toolbar ({
                ...
                items: [{
                    ...
                    items: [{
                        ...
                        text: 'Main Screen',
                        handler: function(){
                            Ext.Msg.alert('Message','Sorry, you cannot change menu during in Upload PR Screen. <br>Please choose action "Send or Save or Cancel Create PR".');
                        }   
                    }]
                },{
                    xtype: 'buttongroup',
            ...
            var itemNameGet;
            var prNoVal;
            var urlPrDetail         = '<?php echo "../db/PR/READ_PR.php?uploadId=".$uploadId."&param=prUpload" ?>';
                ...
                success: function(response,opts,store){
                    ...
                    for(q=0; q < countItem; q++){
                        ...
                        myItem.push([itemCd,itemName,remark,deliveryDate,itemType
                                    ...
                                    ,itemStatus,reasonToReject,rejectItemBy,rejectItemName]);
                        prDetailGrid.store.load();
                    }
                    prNo.setValue(prNoVal);
                    oldPrNo.setValue(prNoVal);
                }
            });
            /** 
             * ========================================================================================================
                ...
                Ext.Ajax.request({
                    ...
                    success: function(response){
						...
						if(answer == 'Defined'){
                            ...
                            Ext.getCmp('sendButton').enable(true);
                            Ext.getCmp('saveButton').enable(true);
                            Ext.getCmp('calculateButton').disable(true);
                            var xs = getApproverList(countApp, currentApp);
                            if(xs) {
            ...
            function deletePrDetail(prDetailGrid,rowIndex,colIndex){
                ...
                if(rowCount==1){
                    Ext.MessageBox.alert('Message','Delete item failed because left only PR Item List.');
                }else{
                     Ext.Msg.show({
                        title: 'Confirm',
						...
						if(qty <= 0 || itemPrice <= 0 || amount <=0 ){
							...
							if(action=='Edit'){
								...
								if(id=='2'){ // cek apakah value di grid == value di form
									myItem[oldItemName]=[itemCd,itemName,remark,deliveryDate,itemType,rfiNo,accountNo,currencyCd,supplierCd,supplierName,unitCd,qty,itemPrice,amount,itemStatus,reasonToReject,rejectItemBy,rejectItemName];
									prDetailGrid.store.load();
									calculateApprover();
								...
								if(id=='2'){
									myItem.push([itemCd,itemName,remark,deliveryDate,itemType,rfiNo,accountNo,currencyCd,supplierCd,supplierName,unitCd,qty,itemPrice,amount,itemStatus,reasonToReject,rejectItemBy,rejectItemName]);
									prDetailGrid.store.load();
									calculateApprover();
                ...
                if(panelCenter.getForm().isValid()){
                    if((Ext.String.trim(purpose.getValue())).length > 0){
						if(dsPrItem.getCount() > 0){
										...
										Ext.Ajax.request({
											...
											success: function(response){
												...
												if(msg=='success_create'){
													...
													if(msg=='failed_create'){
														Ext.MessageBox.alert({
															title   : 'Message',
															msg     : 'PR number has been used by another user. Please, click ok to get new PR Number!',
															buttons : Ext.Msg.OK,
															closable: false,
															fn      : function(btn){
																if(btn=='ok'){
							...
							Ext.MessageBox.alert('Message', 'Please input PR item.');
						}
					}else{
            ...
            function cancelPr(me){
                ...
                Ext.MessageBox.confirm('Message', 'Do you want to ' +me+' ?', function(btn, text){
                    if(btn=='yes'){
                        ...
                        Ext.Ajax.request({
                            method: 'POST',
                            url: '../db/PR/DELETE_PR_ATTACHMENT.php?action=attachmentPr&action2=deleteUpload&uploadId='+<?php echo $uploadId;?>,
                            params: {
                                userIdLoginVal   : userIdLogin.getValue(),
                                oldPrNoVal       : oldPrNo.getValue(),
                                prNo             : prNo.getValue(),
            ...
            var oldPrNo = new Ext.form.TextField({
            ...
            var specialType = Ext.create('Ext.form.RadioGroup',{
                ...
                listeners: {
                    change: function(a){
                        ...
                        if(valSpecialType=='IT' && (Ext.String.trim(valBuCd) != '3300' || Ext.String.trim(valBuCd) != '3941')){
                ...
                listeners: {
                    change: function(){
                        ...
                        if(valItemType=='1'){
                            ...
                            // FA Code
                            /*faCodeForm.setVisible(false);
                            faCodeForm.allowBlank=true;
                            faCodeForm.reset();*/
						...
						else 
						{
                            if(valItemType=='2')
							{
                                ...
                                // FA Code - Show
                                /*faCodeForm.setVisible(true);
                                faCodeForm.allowBlank=false;*/
            ...
            **/
            ...
            var prAttachmentView = Ext.create('Ext.grid.Panel', {
                ...
                columns: [{
                    ...
                    renderer: function (val, metaData, record){
                        ...
                        var userIdVal = userIdLogin.getValue();
                        return '<a href="../db/Attachment/Temporary/'+prNoVal+'-'+userIdVal+'-temp/'+val+'" target="_blank">'+val+'</a>';
                    }
                },{
                    header: 'SIZE',
                    width: 70,
            ...
            var prSpecialApprover = Ext.create('Ext.panel.Panel', {
                ...
                items:[{
                    xtype: 'fieldset',
                    title: '<b>IS Approval</b>',
                    height: 45,
                    width: 310,
                    items: [{
                        xtype: 'textfield',
                        fieldLabel: 'IS Approver',
            ...
            var tb = [{
                ...
                items: [{
                    ...
                    text: 'Cancel Create PR',
                    tooltip: 'Cancel Create PR',
                    id: 'cancelButton',
