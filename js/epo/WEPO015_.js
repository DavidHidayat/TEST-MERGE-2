$(document).ready(function() {
        ...
        buttons     : {
            "Yes": function(){
                ...
                else if(countErrorChceckbox > 0)
                {
                    ...
                    eDataPo = "poNoPrm="+poNo+"&addRemarkPrm="+addRemark+"&npkApproverArray="+poApprover
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            if($.trim(data) == 'Success'){
                                ...
                                $("div#cancel-msg.alert").css('display','none');
                                $("div#update-msg.alert").css('display','block');
                                $("div#close-msg.alert").css('display','none');
                                ...
                                $("button#btn-update.btn.btn-inverse").attr('disabled', true);
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            if($.trim(data) == 'Success'){
                                ...
                                $("div#send-msg.alert").css('display','none');
                                $("div#cancel-msg.alert").css('display','none');
                                $("div#update-msg.alert").css('display','block');
                                $("div#close-msg.alert").css('display','none');
	...
	modal       : true,
        buttons     : {
            "Yes": function(){
                ...
                var addRemark   = $.trim($("#addRemark").val());
                var eDataPo;
                eDataPo     = "poNoPrm="+poNo+"&addRemarkPrm="+addRemark;
                
                $.ajax({
                    type: 'GET',
                    url: '../db/PO/CREATE_PO.php?action=UpdatePoAfterSent',
                    data: eDataPo,
                    success: function(data){
                        console.log(data);
                        if($.trim(data) == 'Success'){
                            ...
                            $("div#send-msg.alert").css('display','none');
                            $("div#cancel-msg.alert").css('display','none');
                            $("div#update-msg.alert").css('display','none');
                            $("div#close-msg.alert").css('display','block');
                            
                            $("button#btn-send.btn.btn-primary").attr('disabled', true);
                            $("button#btn-save.btn.btn-success").attr('disabled', true);
                            $("button#btn-cancel.btn.btn-danger").attr('disabled', true);
                            $("button#btn-update.btn.btn-danger").attr('disabled', true);
                            $("#addRemark").attr('readonly', true);
                            $("#deliveryDate").attr('readonly', true);
	...
	modal       : true,
        ...
        buttons     : {
            "Save"  : function(){
                ...
                var itemStatus      = $.trim($("#itemStatusHidden").val());
                var totalReceived   = $("#totalReceivedHidden").val();
                ...
                edata = "supplierCdSetPrm="+supplierCdSet+"&supplierCdPrm="+supplierCd+"&supplierNamePrm="+supplierName
                        ...
                        +"&seqItemPrm="+seqItem+"&refTransferIdPrm="+refTransferId+"&itemStatusPrm="+itemStatus
                        +"&totalReceivedPrm="+totalReceived;
                    
                $.ajax({
                    type: 'GET',
                    url: '../db/PO/UPDATE_ITEM_SESSION.php?action=EditItemPoSent',
                    data: edata,
                    success: function(data){
                        //alert($.trim(data));
        ...
        $.ajax({
            ...
            success: function(data){
                if($.trim(data).substr(0,7) == 'Success')
                {
                    ...
                    var newCountApp = $("#countApp").val();
                    var initialCountApp = $("#initialCountApp").val();
                    
                    if(initialCountApp == newCountApp){
                        $("button#btn-send.btn.btn-primary").attr('disabled', true);
                        $("button#btn-save.btn.btn-success").attr('disabled', true);
                        $("button#btn-update.btn.btn-inverse").attr('disabled', false);
                    }
                    else
                    {
                        for(var i = 0; i < countApp; i++){
    ...
    $("table .btn-success").click(function(){
        ...
        var itemStatus  = $.trim($(this).closest('tr').find('td:eq(3)').text());
        var totalReceived= $.trim($(this).closest('tr').find('td:eq(15)').text());
        
        $("#refTransferIdHidden").val('');
        $("#currencyCdHidden").val('');
        $("#itemStatusHidden").val('');
        $("#supplierCd").val('');
        $("#supplierName").val('');
        $("#itemCd").val('');
        $("#itemName").val('');
        ...
        $("#price").val(price);
        $("#seqItem").val(seqItem);
        $("#totalReceivedHidden").val(totalReceived);
