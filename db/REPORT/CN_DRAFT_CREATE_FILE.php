...
if($currentCnMonth != '')
{
    ...
    class PDF_MC_Table extends FPDF
    {
        ...
        function WatermarkCn()
        {
            //Put the watermark
            $this->SetFont('helvetica','B',40);
            $this->SetTextColor(0,0,0);
            $this->RotatedText(55,37,'CN DRAFT',0);
        }
    ...
    $dirCN= $_SERVER['DOCUMENT_ROOT']."/".substr($_SERVER['REQUEST_URI'],1,strpos(substr($_SERVER['REQUEST_URI'],1),'/'))."/db/REPORT/CN_DRAFT/";
    ...
    $query_select_t_po_header = "select     
                                    ...
                                    left join
                                        EPS_M_COMPANY 
                                    on 
                                        EPS_T_PO_HEADER.COMPANY_CD = EPS_M_COMPANY.COMPANY_CD 
                                    where     
                                        EPS_T_PO_HEADER.CLOSED_PO_MONTH = '$currentCnMonth'
                                    group by 
                                        EPS_T_PO_HEADER.SUPPLIER_CD
                                        ,EPS_T_PO_HEADER.SUPPLIER_NAME
                                        ,EPS_T_PO_HEADER.COMPANY_CD
    ...
    {
        $supplierCd         = $row_select_t_po_header['SUPPLIER_CD'];
        $supplierName       = $row_select_t_po_header['SUPPLIER_NAME'];
        $supplierNumber     = $row_select_t_po_header['SUPPLIER_NUMBER']; 
        $companyCd          = $row_select_t_po_header['COMPANY_CD'];
        $companyNameVal     = $row_select_t_po_header['COMPANY_NAME'];
        $vatVal             = $row_select_t_po_header['VAT'];
        $currencyCdVal      = $row_select_t_po_header['CURRENCY_CD'];
        $emailVal           = $row_select_t_po_header['EMAIL'];
        ...
        $query_select_t_po = "select
                                ...
                                ,CONVERT(VARCHAR(24), EPS_T_PO_HEADER.CLOSED_PO_DATE, 120) as CLOSED_PO_DATE
        ...
        while($row_select_t_po = $sql_select_t_po->fetch(PDO::FETCH_ASSOC))
        {
            ...
            $split_total_amount = explode('.', $amount);
        ...
        **********************************************************************/
        ...
        $mailCc         = "KARYOTO@taci.toyota-industries.com,WIHARYO@taci.toyota-industries.com,muh.iqbal@taci.toyota-industries.com,ahmadjafar@taci.toyota-industries.com,ISON@taci.toyota-industries.com";

        $mailTo     = $supplierMail;
        $mailSubject  = "[EPS] CREDIT NOTES DRAFT ".$currentCnMonth." ".$supplierName;
        $mailMessage  = "";
