...
if($fileType=="application/vnd.ms-excel"){
    ...
    for ($x=2; $x <= $rowData; $x++)
    {
       ...
       /** Search Item Price */
        ...
        $sql_item_price = $conn->query($query_item_price);
        $row_item_price = $sql_item_price->fetch(PDO::FETCH_ASSOC);
                                                ...
                                                if($row_unitCd)
                                                {
                                                        ...
                                                        {   
                                                            if($row_item){
                                                                if($row_item_price){
                                                                    $uploadItemStatus = 'E';
                                                                    $msgError = 'Duplicate Error. Item Code already exist in Master Item Price.';
                                                                    $countError++;
                                                                }
                                                            }
                                                            else
                                                            {
                                                                $uploadItemStatus = 'E';
                                                                $msgError = 'Existence Error. Item Code is not found Master.';
                                                                $countError++;
                                                            }
                                                        }else{
                                                            $uploadItemStatus = 'E';
        ...
        while($row_upload = $sql_upload->fetch(PDO::FETCH_ASSOC)){
            ...
            $html.="<tr><td>".$i.".</td><td>".$itemCd."</td><td>".$unitCd."</td><td>".$itemPrice."</td><td>".$supplierCd."</td><td>".$effectiveDate."</td><td>".$msgError."</td></tr>";
            $i++;
        }
    ...
    <head>
        ...
        var mainLayout = function(){
            ...
            var panelCenter = Ext.create('Ext.form.Panel', {
                border: false,  
                bodyPadding: '15',
                title: 'Upload Update Item Price Master',
                html: '<form method="post" enctype="multipart/form-data" action="WMST010.php" accept="application/msexcel">'
                        +'<div>'
                        ...
                        +'</div>'
                      +'</form>'
                      +'<br>*Note: Click <a href="../lib/FORMAT_UPLOAD_UPDATE_PRICE_LIST.xls">here</a> to download example file to upload update Item Price Master'
                      +'<?php echo '<br>'.$html; ?>'
            });
