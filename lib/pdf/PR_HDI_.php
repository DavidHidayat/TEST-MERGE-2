...
class PDF_MC_Table extends FPDF
{
        function Header()
        {
            ...
            {  
                //Logo
                $this->Image('fpdf16/tutorial/hdi.jpg',10,10,20);
	...
	function NbLines($w,$txt)
	{
        ...
        function headerTabelItem()
        {
            ...
            $this->Cell(30,7,'Nama Barang','LTR',0,'C',1); 
            $this->Cell(40,7,'Part No, Spec, Ukuran/Dimensi,','LTR',0,'C',1); 
            $this->Cell(20,7,'Merk','LTR',0,'C',1); 
            $this->Cell(10,7,'Rutin/','LTR',0,'C',1); 
            $this->Cell(15,7,'Subtitusi','LTR',0,'C',1); 
            $this->Cell(15,7,'TGL Kirim','LTR',0,'C',1); 
            $this->Cell(16,7,'Charge ke','LTR',0,'C',1); 
            $this->Cell(10,7,'No.','LTR',0,'C',1); 
            $this->Cell(10,7,'Qty','LTR',0,'C',1); 
            $this->Cell(15,7,'U/M','LTR',0,'C',1); 
            $this->Cell(50,7,'Referensi User','LTR',0,'C',1); 
            $this->Cell(23,7,'Alasan Membeli','LTR',1,'C',1); 

            $this->Cell(7,5,'','LRB',0,'C',1);               //no.
            $this->Cell(16,5,'','LRB',0,'C',1);              //nomor pp
            $this->Cell(30,5,'','LRB',0,'C',1);              //nama barang
            $this->Cell(40,5,'Warna dll','LRB',0,'C',1);              
            $this->Cell(20,5,'','LRB',0,'C',1);              
            $this->Cell(10,5,'Tidak','LRB',0,'C',1);         //rutin/tidak
            $this->Cell(15,5,'Boleh/Tidak','LRB',0,'C',1);   //subtitusi
            $this->Cell(15,5,'','LRB',0,'C',1);              //tgl kirim
            $this->Cell(16,5,'No BU/Seksi','LRB',0,'C',1);   //charge
            $this->Cell(10,5,'Item','LRB',0,'C',1);          //no. item
            $this->Cell(10,5,'','LRB',0,'C',1);              //qty
            $this->Cell(15,5,'','LRB',0,'C',1);              //um
            $this->Cell(25,5,'Unit Prices',1,0,'C',1); 
...
$requester  = addslashes($row['REQUESTER_NAME']);
$buCd       = $row['BU_CD'];
$buName     = $row['BU_NAME'];
...
while($row = $sql->fetch(PDO::FETCH_ASSOC))
{
	$no++.'.';
        ...
        if($itemType == '3' && $row_select_m_account['ITEM_TYPE_CD'] == '3'){
            $prCharged = 'N1000';
        }
        // set "No. Item" column
	...
	$pdf->SetWidths(array(7,16,90,10,15,15,16,10,10,15,25,25,23));
        $pdf->SetAligns(array('C','L','L','L','L','L','L','C','C','C','R','C','R','L','L','L'));
	$pdf->Row(array($no,$prNo,$itemName,'','',$deliveryDate,$prCharged,$itemType,$qty,$unitCd,$itemPrice,$supplierName,$remark));
}
$pdf->SetTextColor(0,0,0);
...
$pdf->Cell(204,8,'CATATAN :','LTR',0,'L');
$pdf->SetFont('Times', '', 7);
...
$pdf->Cell(197,6,'',0);
$pdf->Cell(40,6,'PT. HAMADEN INDONESIA MFG',0,1,'R');
