...
include $_SERVER['DOCUMENT_ROOT']."/".substr($_SERVER['REQUEST_URI'],1,strpos(substr($_SERVER['REQUEST_URI'],1),'/'))."//db/PR/EPS_T_PR.php";
include $_SERVER['DOCUMENT_ROOT']."/".substr($_SERVER['REQUEST_URI'],1,strpos(substr($_SERVER['REQUEST_URI'],1),'/'))."//db/Email/PR_EMAIL.php";
include $_SERVER['DOCUMENT_ROOT']."/".substr($_SERVER['REQUEST_URI'],1,strpos(substr($_SERVER['REQUEST_URI'],1),'/'))."//db/Email/SendEmail.php";
...
if (isset($_SESSION['sNPK'])) {
    ...
    if ($sNPK != '') {
        ...
        $prStatus = '';
        $dirNameTemp = $_SERVER['DOCUMENT_ROOT']."/".substr($_SERVER['REQUEST_URI'],1,strpos(substr($_SERVER['REQUEST_URI'],1),'/'))."//db/Attachment/Temporary/" . $prNo;
        $dirName = $_SERVER['DOCUMENT_ROOT']."/".substr($_SERVER['REQUEST_URI'],1,strpos(substr($_SERVER['REQUEST_URI'],1),'/'))."//db/Attachment/Fixed/" . $prNo;
        /**
        ...
        if (!$currentPrNo) {
            ...
            for ($x = 0; $x < count($prItemData); $x++) {
                ...
                $remark2 = $prItemData[$x][3];
                $remark2 = str_replace("'", "''", $remark2);
                $remark2 = preg_replace("/\r\n+|\r+|\n+|\t+/i", " ", $remark2);
                $remark2 = preg_replace('/\s+/', ' ', $remark2);
                $deliveryDate = encodeDate($prItemData[$x][4]);
                $itemType = $prItemData[$x][5];
                $rfiNo = $prItemData[$x][6];
                $accountNo = $prItemData[$x][7];
                $currencyCd = $prItemData[$x][8];
                $supplierCd = trim($prItemData[$x][9]);
                $supllierName = strtoupper(trim($prItemData[$x][10]));
                $supllierName = str_replace("'", "''", $supllierName);
                $supllierName = preg_replace("/\r\n+|\r+|\n+|\t+/i", " ", $supllierName);
                $supllierName = preg_replace('/\s+/', ' ', $supllierName);
                $unitCd = $prItemData[$x][11];
                $qty = $prItemData[$x][12];
                $itemPrice = $prItemData[$x][13];
                $amount = $prItemData[$x][14];
                $rutin = $prItemData[$x][15];
                $subtitusi = $prItemData[$x][16];
                $prChargedItem = $prItemData[$x][17];
                $itemStatus = trim($prItemData[$x][18]);
                $reasonToReject = trim($prItemData[$x][19]);
                $rejectItemBy = $prItemData[$x][20];
                $currencyCd = "IDR";
                ...
                if ($prCharged == 'T6214'|| $prCharged == 'T6215'){
                
                }else{
                    $query_select_m_rfi = "SELECT RFI_NO FROM EPS_M_RFINO 
                                where RFI_NO = '".$rfiNo."'";
        
                                $sql_select_m_rfi = $conn->query($query_select_m_rfi);
                                $row_select_m_rfi = $sql_select_m_rfi->fetch(PDO::FETCH_ASSOC);
                                $rfiNoMaster = $row_select_m_rfi['RFI_NO'];
                                //echo $rfiNoMaster;
                                if($rfiNoMaster != ''){
                                 $query = "INSERT INTO EPS_T_RFI_NO
                                   (RFI_NO
                                   ,USER_MAIL
                                   ,MAIL_SUBJECT
                                   ,SENT
                             ...
                             VALUES
                                   ('$rfiNoMaster'
                                   ,'pe-budgetHR@taci.toyota-industries.com; WIDHIWAHYUNIARTI@taci.toyota-industries.com'
                                   ,'USING YOUR BUDGET BY NO RFI'
                                   ,'0'
                                   ,'$prNo'
                                   ,'$prCharged'
                                   ,'$requesterMailName')";
            ...
            if ($oldPrNo != $prNo) {
                $dirNameTemp = $_SERVER['DOCUMENT_ROOT']."/".substr($_SERVER['REQUEST_URI'],1,strpos(substr($_SERVER['REQUEST_URI'],1),'/'))."//db/Attachment/Temporary/" . $oldPrNo;
            }
