$(document).ready(function() {
     ...
     * =========================================================================================================
     **/
	var newMax = getMaxPoAmount();
    var newCurrencyCd = $.trim($("#currencyCd").val());
    var eMaxAmountNew;
    eMaxAmountNew = "maxPoAmountPrm="+newMax+"&currencyCdPrm="+newCurrencyCd;
    $.ajax({
        type: 'GET',
            ...
            if($.trim(data).substr(0,7) == 'Success')
            {
                ...
                if(countApp > 0){
                    for(var i = 0; i < countApp; i++){
                        if($("input#setApproverSts"+appNoVal).val() == 'BP' && appNoVal < countApp){
                            $("select#approverNo"+appNoVal+".span3").attr('disabled', true);
                            $("input#setBypassNo"+appNoVal).prop('checked', true);
                        }else{
                            $("select#approverNo"+appNoVal+".span3").attr('disabled', false);
                            $("input#setRemarkNo"+appNoVal+".span5").val('');
                        }
                        $("select#approverNo"+(i+1)+".span3 option[value='"+$("#setApproverNpk"+(i+1)).val()+"']").prop("selected", true);
    ...
    $("input[type='checkbox']").click(function () {
        ...
        else if(checkedVal == true && approverVal != '')
        {
            ...
            $("#setRemarkNo"+index).attr('readonly', false);
            $("select#approverNo"+index+".span3").attr('disabled', true);
            $("button#btn-save-bypass-"+index+".btn").attr('disabled', false);  
        }
        else{
            $("div#mandatory-msg-3.alert").css('display','none');
            $("#setApproverSts"+index).val('');
            $("#setRemarkNo"+index).val('');
            $("#setRemarkNo"+index).attr('readonly', true);
        ...
        buttons     : {
            "Save"  : function(){
                ...
                edata = "supplierCdSetPrm="+supplierCdSet+"&supplierCdPrm="+supplierCd+"&supplierNamePrm="+supplierName
                        ...
                        +"&prItemPricePrm="+prItemPrice+"&limitPricePrm="+limitPrice+"&cipPrm="+cip;
				...
				"Yes": function(){
					...
					for(var i = 0; i < countApp; i++){
						...
						if($("input#setBypassNo"+appNoVal).prop('checked') == true
							&& $.trim($("input#setRemarkNo"+appNoVal).val()) == ''){
							countErrorChceckbox++;
						}
						appNoVal++;
					...
					else
					{
						for(var j = 0; j < countApp; j++){
							...
							/** Get approver bypass remark value */
							if($("input#setBypassNo"+appNoVal2).prop('checked') == true){
								var bypassReason = $.trim($("input#setRemarkNo"+appNoVal2).val());
								bypassApprover.push(appNoVal2+bypassReason);
							}
							appNoVal2++;
						}
						eDataPo = "poNoPrm="+poNo+"&deliveryDatePrm="+deliveryDate+"&addRemarkPrm="+encodeURIComponent(addRemark)
									+"&npkApproverArray="+poApprover
									+"&bypassApproverArray="+encodeURIComponent(bypassApprover);
						
						$.ajax({
							type: 'GET',
							url: '../db/PO/CREATE_PO.php?action=SavePo',
							data: eDataPo,
							success: function(data){
								//alert(data);
								if($.trim(data) == 'Success'){
									...
									$("input.span5").attr('readonly', true);
			...
			buttons     : {
				"Yes": function(){
					...
					else if(countErrorChceckbox > 0)
					{
						...
						$.ajax({
							type: 'GET',
							url: '../db/PO/CREATE_PO.php?action=SendPo',
							data: eDataPo,
							success: function(data){
								//alert(data);
								if($.trim(data) == 'Success'){
                ...
                success: function(data){
                    if($.trim(data).substr(0,7) == 'Success')
                    {
                        ...
                        if(countApp > 0){
                            ...
                            for(var i = 0; i < countApp; i++){
                                if($("input#setApproverSts"+appNoVal).val() == 'BP'){
                                    $("select#approverNo"+appNoVal+".span3").attr('disabled', true);
                                }else{
                                    $("select#approverNo"+appNoVal+".span3").attr('disabled', false);
                                }
