$(document).ready(function() { 
    ...
    ['#itemPrice', '#leadTime'].forEach(element => {
        if (document.querySelector(element)) {
            document.querySelector(element).addEventListener('keypress',preventNonNumbersInInput);
            document.querySelector(element).addEventListener('paste',pasteTest);                
        }
    });
...
//            $("select#faCd.form-control").attr('disabled', true);
    ...
    $(function(){
        $("#itemName").autocomplete({
            ...
            select      : function (event, ui) {
                if($.trim(ui.item.itemCd) != '99'){
                    ...
                    $('#itemCategory').val(ui.item.itemCategory=="Y"?"ROUTINE":"NON ROUTINE");
        ...
        $("#itemName[role='master']").autocomplete({
            source      : '../db/MASTER/EPS_M_ITEM.php?action=search',
            minLength   : 2,//search after two characters
            select: function (event, ui) {
                if($.trim(ui.item.itemCd) != '99'){
                    $('#itemCd').val(ui.item.itemCd);
	...
	height      : 570,
	width       : 780,
        position    : {my: "top middle", at: "top middle", of: $("#prItemTable"), within: $("#prItemTable")},
	modal       : true,
        open        : function() {                         // open event handler
            $(this)                                // the element being dialogged
                ...
                .hide();                           // hide it
            $('.ui-dialog[aria-describedby="dialog-form-item"]')[0].style.top = '100px';
        },
        buttons     : {
            "Save"  : function(){
                var edataItem;
                ...
                var itemCategory        = $.trim($("#itemCategory").val()).toUpperCase();
                ...
                edataItem = "itemSeqHiddenPrm="+itemSeqHidden+"&itemTypePrm="+itemType
                            ...
                            +"&itemCategoryPrm="+encodeURIComponent(itemCategory)+"&actionPrm="+action;
