...
if(isset($_SESSION['sUserId']))
{
    ...
    {   
        ...
        if($action == 'SEARCH')
        {
            ...
            $query = "select ITEM_CD, ITEM_NAME, SUPPLIER_CD, SUPPLIER_NAME
            from         
                EPS_M_ITEM
            left join
                EPS_M_SUPPLIER 
            on 
                EPS_M_SUPPLIER.SUPPLIER_CD = SUBSTRING(ITEM_CD,10,4)
            where     
                ITEM_NAME LIKE '%" . trim($itemName) . "%'";
               $sql = $conn->query($query);
            $result = array();
            while ($row = $sql->fetch(PDO::FETCH_ASSOC)) {
                $data = array();
                foreach ($row as $key => $value) {
                    $data[stringToCamelCase($key)] = $value;
                }
                $data['id'] = $row['ITEM_CD'];
