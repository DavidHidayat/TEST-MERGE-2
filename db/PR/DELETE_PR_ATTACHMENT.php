        ...
        if($action=='attachmentItem'){
            ...
            for($x=0 ; $x < count($itemAttachment); $x++){
                ...
                if($actionPr == 'Crea' || $actionPr == 'Uplo' || $actionPr == 'Repl'){
                    if($oldPrNo != $prNo){
                        unlink($_SERVER['DOCUMENT_ROOT']."/EPS/db/Attachment/Temporary/".$oldPrNo.'-'.$userId.'-temp/'.$fileName);
                    }else{
                        unlink($_SERVER['DOCUMENT_ROOT']."/EPS/db/Attachment/Temporary/".$prNo.'-'.$userId.'-temp/'.$fileName);
                    }
                }else{
                    unlink($_SERVER['DOCUMENT_ROOT']."/EPS/db/Attachment/Temporary/".$prNo.'-temp/'.$fileName);
                }
            }
        }
