    ...
    if($sUserId != '' && $sActiveFlag == 'A' && $sActiveFlagLogin == 'A')
    {
        ...
        if($sRoleId == 'ROLE_03' || $sRoleId == 'ROLE_04' || $sRoleId == 'ROLE_05' || $sRoleId == 'ROLE_06' || $sRoleId == 'ROLE_09' || $sRoleId == 'ROLE_11')
        {
                    ...
                    || $itemStatusCriteria || $prChargedCriteria || $poStatusCriteria || $roStatusCriteria)
            {
                ...
                $whereRoSelect = array();
                if($poNoCriteria){
                    $whereRoSelect[] = "EPS_T_PO_DETAIL.PO_NO = '".$poNoCriteria."'";
                }
                if($poDateCriteria && !$poDateEndCriteria){
                    $whereRoSelect[] = "EPS_T_PO_HEADER.ISSUED_DATE = '".encodeDate($poDateCriteria)."'";
                }
                if(!$poDateCriteria && $poDateEndCriteria ){
                    $whereRoSelect[] = "EPS_T_PO_HEADER.ISSUED_DATE = '".encodeDate($poDateEndCriteria)."'";
                }
                if($poDateCriteria && $poDateEndCriteria){
                    $whereRoSelect[] = "EPS_T_PO_HEADER.ISSUED_DATE >= '".encodeDate($poDateCriteria)."'
                ...
                if($deliveryDateCriteria){
                    $whereRoSelect[] = "EPS_T_PO_HEADER.DELIVERY_DATE = '".encodeDate($deliveryDateCriteria)."'";
                }
                if($prNoCriteria){
                    $whereRoSelect[] = "EPS_T_TRANSFER.PR_NO = '".$prNoCriteria."'";
                }
                if($requesterCriteria){
                    $whereRoSelect[] = "EPS_M_EMPLOYEE.NAMA1 like '".$requesterCriteria."%'";
                }
                if($supplierCdCriteria){
                    $whereRoSelect[] = "EPS_T_PO_HEADER.SUPPLIER_CD = '".$supplierCdCriteria."'";
                }
                if($supplierNameCriteria){
                    $whereRoSelect[] = "EPS_T_PO_HEADER.SUPPLIER_CD = '".$supplierNameCriteria."'";
                }
                if($itemNameCriteria){
                    $whereRoSelect[] = "EPS_T_PO_DETAIL.ITEM_NAME LIKE '%".$itemNameCriteria."%'";
                }
                if($itemTypeCriteria){
                    $whereRoSelect[] = "EPS_T_TRANSFER.NEW_ITEM_TYPE_CD = '".$itemTypeCriteria."'";
                }
                if($expNoCriteria){
                    $whereRoSelect[] = "EPS_T_TRANSFER.NEW_ACCOUNT_NO = '".$expNoCriteria."'";
                }
                if($invNoCriteria){
                    $whereRoSelect[] = "EPS_T_TRANSFER.NEW_ACCOUNT_NO = '".$invNoCriteria."'";
                }
                if($rfiNoCriteria){
                    $whereRoSelect[] = "EPS_T_TRANSFER.NEW_RFI_NO = '".$rfiNoCriteria."'";
                }
                if($itemStatusCriteria){
                    $whereRoSelect[] = "EPS_T_TRANSFER.ITEM_STATUS = '".$itemStatusCriteria."'";
                }
                if($prChargedCriteria){
                    $whereRoSelect[] = "EPS_T_TRANSFER.NEW_CHARGED_BU = '".$prChargedCriteria."'";
                }
                if($poStatusCriteria){
                    $whereRoSelect[] = "EPS_T_PO_HEADER.PO_STATUS = '".$poStatusCriteria."'";
                }
                if($roStatusCriteria){
                    $whereRoSelect[] = "EPS_T_PO_DETAIL.RO_STATUS = '".$roStatusCriteria."'";
                }
				
                /**
                 * SELECT COUNT EPS_T_PO
                 **/
                $query_count_t_ro = "select 
                                        count (*) as COUNT_RO
                                    from
                                        EPS_T_PO_DETAIL 
                                    left join
                ...
                if(count($whereRoSelect)) {
                    $query_count_t_ro .= "where " . implode(' and ', $whereRoSelect);
                }
                $sql_count_t_ro = $conn->query($query_count_t_ro);
                $row_count_t_ro = $sql_count_t_ro->fetch(PDO::FETCH_ASSOC);
                $countRo    = $row_count_t_ro['COUNT_RO'];
                
                if($start > $countRo)
                {
                    $lgenap     = $start - $max_per_page;
                    $max_per_pages = $countRo - $lgenap;
                    $start      = $countRo;
                }
                else
                {
                        ...
                        from 
                            ...
                            from      
                                (select top $start 
                                    ...
                                    ,EPS_M_EMPLOYEE_2.NAMA1 as PO_ISSUED_NAME
                ...
                if(count($whereRoSelect)) {
                    $query_t_ro_select .= "where " . implode(' and ', $whereRoSelect);
                }
                /*$query_t_ro_select .= " 
                                order by 
                                    EPS_T_PO_HEADER.SUPPLIER_NAME
                            ...
                            order by
                                ...
                                ,T1.REF_TRANSFER_ID desc
                                ,T1.DELIVERY_DATE desc)
                            as T2
                        order by
                            T2.SUPPLIER_NAME asc
                            ,T2.REF_TRANSFER_ID 
                            ,T2.DELIVERY_DATE ";*/
				$query_t_ro_select .= " 
                                order by 
                ...
                $row_t_ro_select = $sql_t_ro_select->fetch(PDO::FETCH_ASSOC);
                //echo $query_t_ro_select;
            }
        }
        else
        {
...
<html>
    <head>
        ...
        <script src="../js/ero/WERO090.js"></script>
        <script>
            $(function() {
        ...
        <div class="container">
            <div class="row">
                <div class="span12"> 
                    ...
                    <?php
                    if($poNoCriteria || $supplierCdCriteria || $poDateCriteria || $deliveryDateCriteria || $roStatusCriteria
                        || $prNoCriteria || $requesterCriteria || $supplierCdCriteria || $itemNameCriteria)
                    {
                        if(!$row_t_ro_select)
                        {
                    ?>
                                    ...
                                    <table class="table-non-bordered">
                                        <tr>
                                            ...
                                            <td colspan="2">
                                                ...
                                                <div class="controls">
                                                    <input type="text" class="span2" id="requester" name="requesterCriteria" maxlength="7" value="<?php echo $requesterCriteria;?>" />
                                                </div>
                                            </td>
                        ...
                        <div class="widget widget-table action-table">
                            ...
                            <div class="widget-content">
                                <table class="table table-striped table-bordered">
                                    ...
                                    <?php
                                    $poListNo = 0;
                                    $sql_t_ro_select_2 = $conn->query($query_t_ro_select);
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php echo substr($poIssuedName, 0, strpos($poIssuedName, ' '));?>
                                            </td>
                                            <td>
                                            ...
                                            <td class="td-align-right">
                                                <?php echo $totalReceivedQty;?>
                                            </td>
                                            <td class="td-align-right">
                                                <?php echo $totalOpenQty;?>
                                            </td>
                                    ...
                                    ?>
										<tr>
                                            <th colspan="18">
                                            <?php
                                                if($countRo > $max_per_page)
                                                {    
                                                    echo "<div id=\"pagination\" >";
                                                    if ($query_t_ro_select != "")
                                                    {
                                                    ...
                                                    paging($query_t_ro_select,$max_per_page,$num,$mpage,$fld,$countRo);
                                                    echo "</div>";
                                                }
                                            ?>
                                            </th>
