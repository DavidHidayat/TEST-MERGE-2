$(document).ready(function() {
	...
	modal       : true,
        ...
        buttons     : {
            "Save"  : function(){
                ...
                var cancelQty       = $.trim($("#cancelQty").val());
                var cancelDate      = $.trim($("#cancelDate").val());
                ...
                edata = "poNoPrm="+poNo+"&refTransferIdPrm="+refTransferId
                        ...
                        +"&cancelQtyPrm="+cancelQty+"&cancelDatePrm="+cancelDate
                        +"&cancelRemarkPrm="+encodeURIComponent(cancelRemark)
                        +"&roSeqPrm="+roSeq+"&actionPrm="+action;
                
                $.ajax({
                    type: 'GET',
                    url: '../db/RO/UPDATE_RECEIVED_SESSION.php?action=UpdateCancelReceiving',
                    data: edata,
        ...
        $("#dialog-confirm-save").dialog({
            ...
            buttons     : {
                "Yes": function(){
                    ...
                    $.ajax({
                        type: 'GET',
                        url: '../db/RO/CREATE_RO.php?action=CancelReceiving',
                        data: edataItem,
                        success: function(data){
                            //alert($.trim(data));
                            ...
                            if($.trim(data) == 'Success')
                            {
                                ...
                                $("a#window-cancel.btn.btn-small.btn-danger").css('display', 'none');
								
								window.location = "WERO006.php?poNo="+poNo;
                            }
                            else if($.trim(data) == 'Mandatory_1')
                            {
    ...
    $("table .btn-danger").click(function() {
        ...
        $("#cancelQty").val('');
        $("#cancelDate").val(currentDate);
        $("#roSeq").val(rowCount);
        $("#dialog-form").dialog('option', 'title', 'Cancel Received Qty');
